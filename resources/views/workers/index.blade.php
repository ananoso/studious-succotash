            ...
            <div class="overflow-x-auto w-full">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase bg-gray-50 border-b">
                        ...
                        <th class="px-4 py-3">Type of employment</th>
                    ...
                    <tbody class="bg-white divide-y">
                        ...
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3 text-sm">{{ $worker->surname }}</td>
                            <td class="px-4 py-3 text-sm">{{ $worker->phone }}</td>
                            <td class="px-4 py-3 text-sm">{{ $worker->specialization }}</td>
                            <td class="px-4 py-3 text-sm">{{ $worker->type_of_employment }}</td>
                            <td>
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('workers.edit', $worker->id) }}" class="text-blue-500 hover:underline">
                                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                        </svg>
                                    </a>
                                    <form action="{{ route('workers.destroy', $worker->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
